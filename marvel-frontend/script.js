...
btnListAll.addEventListener('click', (e) => {
    ...
    xhttp.open('GET', 'http://127.0.0.1:8558/api/characters', true);
    xhttp.send();
...
function characterOnClickHandle(characId) {
    ...
    xhttp.open('GET', `http://127.0.0.1:8558/api/characters/${characId}`, true);
    xhttp.send();
}
...
btnListAllMovies.addEventListener('click', (e) => {
    ...
    xhttp.open('GET', 'http://127.0.0.1:8558/api/movies', true);
    xhttp.send();
    // Open(): Specifies the type of request
    // send(): Sends the request to the server (used for GET)
});
...
function movieOnClickHandle(movieId) {
    ...
    xhttp.open('GET', `http://127.0.0.1:8558/api/movies/${movieId}`, true);
    xhttp.send();
}